Replication script to display the replication topology of the customer's environment: 
	1. This script needs to be run on the distribution database on the distributor server. 
	2. The script will display the replication topology at the customer’s server (output can be taken in a file also) and can be used for the following purpose: 
		a. To understand the distributor server (along with build), publisher server and subscriber server information including the names of the server instances, publication, subscription databases 
		b. To understand the number of publisher servers, number of publications and subscriptions in the entire replication topology 
		c. To scope the case on a specific publication-subscription pair. The Engineer can copy the publication-subscription information from the display and send the scoping mail with the information along with the information pasted in the case notes. 
